...
if (-not $CodexHome -and $env:CODEX_HOME) {
...
if ($PSBoundParameters.ContainsKey('Wsl')) {
    ...
    if (-not $CodexHome) {
        ...
        $CodexHome = Join-Path $WslHome.Trim() ".codex"
    }
    $UnixPath = $CodexHome.Trim() -replace '^/', ''
    $TargetBase = "\\wsl$\$Distro\$UnixPath\skills"
    Write-Host "Targeting WSL distro '$Distro': $TargetBase"
} elseif ($CodexHome) {
    $TargetBase = Join-Path $CodexHome "skills"
} else {
    $TargetBase = Join-Path $env:USERPROFILE ".codex\skills"
}
